...
export default function addPrisma({ project }: { project: Project }) {
  ...
  if (
    packageJsonParsed.devDependencies["prisma"] ||
    ...
    if (
      packageJsonParsed.devDependencies["@prisma/client"] ||
  ...
  const prismaDir = glob.sync("**/prisma/", {
    ignore: ["**/node_modules/**"],
  });
  ...
  const prismaFile = glob.sync("./prisma/schema.prisma", {
    ignore: ["**/node_modules/**"],
  });
  if (prismaFile.length > 0) {
    ...
    const prisma = project.createSourceFile(
  ...
  const existingPrismaClient = project.getSourceFiles().find((file) => {
    return file
      .getChildrenOfKind(ts.SyntaxKind.ImportDeclaration)
      .find((node) => {
  ...
  if (existingPrismaClient) {
    ...
    console.log(
      "  Existing prisma client not found, creating prisma client..."
    );
    const prismaClient = project.createSourceFile(
    ...
    prismaClient.save().then(() => {
      const prismaClientFile = glob.sync("./server/utils/prisma.ts", {
        ignore: ["**/node_modules/**"],
      });
