...
export default function addtRPC({
  project,
  prisma,
}: {
  project: Project;
  ...
  if (
    packageJsonParsed.devDependencies["zod"] ||
  ...
  if (
    packageJsonParsed.devDependencies["@trpc/server"] ||
  ...
  const tRPCDir = glob.sync("**/api/trpc/", {
    ignore: ["**/node_modules/**"],
  });
  if (tRPCDir.length === 0) {
    ...
    console.log(
      "  /api/trpc directory already exists, located at:",
      tRPCDir[0]
    );
  }
  ...
  if (tRPCFile) {
    ...
    if (prisma) {
      const tRPC = project.createSourceFile(
        "./api/trpc/[trpc].ts",
        `import { createRouter, inferAsyncReturnType } from "@trpc/server";
...
`
    ...
    createRouter.save().then(() => {
      const createRouterFile = project.getSourceFile(
        "./server/utils/createRouter.ts"
      );
      if (createRouterFile) {
    ...
    const context = project.createSourceFile(
