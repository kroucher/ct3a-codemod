...
function addNextAuth({ project }) {
    ...
    const nextAuthDir = glob_1.glob.sync("**/api/auth/", {
        ignore: ["**/node_modules/**"],
    });
    if (nextAuthDir.length === 0) {
        ...
        console.log("api/auth directory already exists, located at:", nextAuthDir[0]);
    }
    ...
    if (nextAuthFile) {
        ...
        const nextAuth = project.createSourceFile("[...nextauth].ts", `import { NextApiRequest, NextApiResponse } from "next";
        ...
        nextAuth.save().then(() => {
            ...
            if (newNextAuthFile) {
                ...
                if (nextAuthDir.length === 0) {
                    ...
                    console.log("api/auth directory already exists, located at:", nextAuthDir[0]);
                    newNextAuthFile.moveImmediatelySync(`${nextAuthDir[0]}[...nextauth].ts`);
                }
            }
            else {
